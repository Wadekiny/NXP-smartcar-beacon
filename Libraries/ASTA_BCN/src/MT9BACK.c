...
//-------------------------------------------------------------------------------------------------------------------
void mt9back_cof_uart_interrupt2(void)
{
    uart_getchar(MT9BACK_COF_UART,&receive2[receive_num2]);;
    receive_num2++;
    
    if(1==receive_num2 && 0XA5!=receive[0])  receive_num2 = 0;
    if(3 == receive_num2)
    {
        receive_num2 = 0;
...
void get_config2(void)
{
    ...
    for(i=0; i<CONFIG_FINISH-1; i++)
    {
        ...
        uart_putbuff(MT9BACK_COF_UART,send_buffer,4);
...
void camera_init(void)
{
    ...
    uart_init (MT9BACK_COF_UART, 9600);	//初始换串口 配置摄像头    
    uart_rx_irq_en(MT9BACK_COF_UART);
