...
uint8   image2[ROW][COL];      //图像数组
uint8   receive2[3];
uint8   receive_num2 = 0;
uint8   uart_receive_flag2 = 1;
//需要配置到摄像头的数据
...
int16 MT9V032_CFG2[CONFIG_FINISH][2]=
{
    {AUTO_EXP,          0},   //自动曝光设置      范围1-63 0为关闭 如果自动曝光开启  EXP_TIME命令设置的数据将会变为最大曝光时间，也就是自动曝光时间的上限
                              //一般情况是不需要开启这个功能，因为比赛场地光线一般都比较均匀，如果遇到光线非常不均匀的情况可以尝试设置该值，增加图像稳定性
    {EXP_TIME,          500}, //曝光时间          摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
    {FPS,               100},  //图像帧率          摄像头收到后会自动计算出最大FPS，如果过大则设置为计算出来的最大FPS
    {SET_COL,           COL2}, //图像列数量        范围1-752     K60采集不允许超过188
    {SET_ROW,           ROW2}, //图像行数量        范围1-480
    {LR_OFFSET,         0},   //图像左右偏移量    正值 右偏移   负值 左偏移  列为188 376 752时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    {UD_OFFSET,         0},   //图像上下偏移量    正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    {GAIN,              32},  //图像增益          范围16-64     增益可以在曝光时间固定的情况下改变图像亮暗程度
...
{
    uart_getchar(MT9V032_COF_UART2,&receive2[receive_num2]);;
    receive_num2++;
    
    if(1==receive_num2 && 0XA5!=receive2[0])  receive_num2 = 0;
    if(3 == receive_num2)
    {
        receive_num2 = 0;
...
void get_config2(void)
{
    ...
    for(i=0; i<CONFIG_FINISH-1; i++)
    {
        ...
        uart_putbuff(MT9V032_COF_UART2,send_buffer,4);
        
        //等待接受回传数据
        while(!uart_receive_flag2);
        uart_receive_flag2 = 0;
        
        GET_CFG2[i][1] = receive2[1]<<8 | receive2[2];
...
uint16 get_version2(void)
{
    ...
    return ((uint16)(receive2[1]<<8) | receive2[2]);
}
//-------------------------------------------------------------------------------------------------------------------
//  @brief      单独设置摄像头曝光时间
...
uint16 set_exposure_time2(uint16 light)
{
	uint16 temp;
    uint8  send_buffer[4];
...
uint16 set_mt9v032_reg2(uint8 addr, uint16 data)
{
	uint16 temp;
    uint8  send_buffer[4];
...
void camera_init2(void)
{
    ...
    uart_init (MT9V032_COF_UART2, 9600);	//初始换串口 配置摄像头    
    uart_rx_irq_en(MT9V032_COF_UART2);
    ...
    {
        send_buffer[0] = 0xA5;
        send_buffer[1] = MT9V032_CFG2[i][0];
        temp = MT9V032_CFG2[i][1];
        send_buffer[2] = temp>>8;
        send_buffer[3] = (uint8)temp;
    ...
    while(!uart_receive_flag2);
    uart_receive_flag2 = 0;
    while((0xff != receive2[1]) || (0xff != receive2[2]));
    //以上部分对摄像头配置的数据全部都会保存在摄像头上51单片机的eeprom中
    //利用set_exposure_time函数单独配置的曝光数据不存储在eeprom中
    ...
    DisableInterrupts;
    //DMA通道0初始化，PTC18触发源(默认上升沿)，源地址为C_IN_DATA(1)(PTC8-PTC15)，目的地址为：image，每次传输1Byte 传输完毕保持目的地址
    dma_portx2buff_init(DMA_CH2, (void *)&MT9V032_DATAPORT2, (void *)image2, MT9V032_PCLK2, DMA_BYTE1, COL2*ROW2, DADDR_KEEPON);
    port_init(MT9V032_PCLK2, ALT1 | DMA_RISING | PULLDOWN);  	//PCLK  触发源设置
	DMA_DIS(DMA_CH2);                                     		//禁用DMA通道
    DMA_IRQ_CLEAN(DMA_CH2);                               		//清除通道传输中断标志位
    DMA_IRQ_EN(DMA_CH2);                                  		//允许DMA通道中断
	DMA_EN(DMA_CH2);  											//使能DMA

	disable_irq(MT9V032_INTERRUPT_NUNBERS2);                     //关闭PTC的中断
    //port_init(C7, ALT1 | IRQ_FALLING | PULLDOWN);      			//行中断
    port_init(MT9V032_VSYNC2, ALT1 | IRQ_FALLING | PULLDOWN);    //场中断，下降沿触发中断、下拉
    set_irq_priority(MT9V032_INTERRUPT_NUNBERS2,1);              // 中断优先级
    enable_irq (MT9V032_INTERRUPT_NUNBERS2);
	EnableInterrupts;
}
//-------------------------------------------------------------------------------------------------------------------
//  @brief      MT9V032摄像头场中断
...
void VSYNC2(void)
{
    MT9V032_INTERRUPT_ISFR2 = 1 << MT9V032_VSYNC_CHANNEL2;
    dma_repeat(DMA_CH2,(void *)&MT9V032_DATAPORT2,(void *)image2[0],COL2*ROW2);
}

uint8   mt9v032_finish_flag = 0;      //一场图像采集完成标志位
uint8   mt9v032_finish_flag2 = 0;      //一场图像采集完成标志位
//-------------------------------------------------------------------------------------------------------------------
